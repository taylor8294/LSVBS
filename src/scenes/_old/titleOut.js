export function titleOut() {
  ...
  var tl = new TimelineLite({ id: 'titleOut' }),
      qty = 25,
      my = window.VBS.pixi.my,
      video = window.VBS.pixi.video,
  ...
  // Text out
  tl.to(my, 1, { x: WIDTH/2, ease: Power2.easeInOut }, '-=5.5') //(t~1.5)
  tl.to(video, 1.5, { x: -WIDTH/8, ease: Power2.easeInOut }, '-=5.5') //(t~1.5)
  tl.to(wtw, 1, { pixi: {alpha:0}, ease: Power2.easeInOut }, '-=4') //(t~3)

  tl.set([my,video], { visible: false }); //(t~7)
  ...
  function waitForSpeechesToLoad(){
    ...
    if(window.VBS.speech.speechesLoaded < window.VBS.speech.speechesToLoad && waitCount < 100){
      if(this.DEBUG) console.log('Video Statement '+this.id+': Waiting for speeches to load...')
